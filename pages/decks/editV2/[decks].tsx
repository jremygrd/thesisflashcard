...
//Fin modal
  ...
  function isCardGood(force:boolean){
    ...
    if(allOptions.length == 0){
      ans = false;
      if(!force)
      handleClick("La carte doit avoir au moins une bonne réponse")
    }
    ...
    if (quest==""){
      ans = false;
      if(!force)
      handleClick("La carte doit avoir une question")
    }
  ...
  const addCard = () => {
    if(lockWait==false )
    {
      if(saveNew.length <3 && isCardGood(false)) // Fix bug et anti flood de la database
      {
        if(saveNew==[""]){
          submitChange();
        }
        ...
        if(saveNew.length >2)
        changeCard(number,false);
      }
    }else // For safety purposes, in case the lock is stuck
    {
        setLockWait(false) ;
    }
  ...
  const delCardDB = async (toDel:any) => {
    ...
    const deletecard = await fetch(`http://localhost:3000/api/editDeck/deletecard`,
        {
          method: "post",
    ...
    if (allOptions.length > 0 && answers.length > 0) {
      ...
      if(saveNew.includes(update[number].id)==false){
        ...
        const upload = await fetch(`http://localhost:3000/api/editDeck/addcard`,
        {
          method: "post",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({question : update[number],user : sessionUser,deck :deckData}),
  ...
  const searcher = (e:any)=>{
    setSearch(e);
    var result = questions.filter((q: { question: string; })  => q.question.toLocaleLowerCase().includes(search.toLocaleLowerCase()))
    if(e == ""){
      result = questions
    };
  ...
  {questions[0]?
    <>
    {searchResults.map((questionsMapped: { question: string, id:string;},idx) => (
      <div onClick={() => changeCard(questions.findIndex((q: { id: string; }) => q.id === questionsMapped.id),false)} 
      
      key={questionsMapped.id} 
      className = {isMobile ? styles.listCardMobile:styles.listCard}> {questionsMapped.question? questionsMapped.question.length > 25 ? questionsMapped.question.slice(0,25)+"..." :questionsMapped.question:null}</div>  
    ))}
    </>
    :null}

    <div key="AddCard" onClick={() => addCard()} className = {isMobile ? styles.listCardMobile:styles.listCardAddCard}> Ajouter une carte</div>
    ...
    <div className={isMobile ? styles.splitupper:styles.splitright}>
      ...
      <>
        ...
        <div className={styles.circledivSave}>
            <IconButton aria-label="grid" onClick={() => changeCard(number,false)}>
          ...
          <TextField id="standard-multiline-static" multiline  fullWidth
          rows={2} label="Question" autoComplete="off" 
        ...
        <div className = {styles.optionWrapper} key = {idx}>
          <div className = {styles.supprEdit}>
            <IconButton key = {idx} aria-label="grid" onClick={() => deleteOption(val)}>
        ...
        <div className = {styles.addOptionCard}>Ajouter une réponse
        ...
        tip.length == 0 ?
        <div className = {styles.addOptionCard}>Ajouter un indice
        ...
        <div className = {styles.optionWrapper}>
          ...
          <div className = {styles.optionEdit}>
              <TextField id="standard-multiline-static" multiline  fullWidth
              rows={1} label="Indice" autoComplete="off" 
        ...
        <div className = {styles.delOptionCard}>Supprimer la carte
        <IconButton aria-label="grid" onClick={delCard} disabled={lockWait}>
        ...
        <>
          <div className = {styles.inputQuestionCard}>
          <TextField id="standard-multiline-static" multiline  fullWidth
          rows={2} label="Deck Title" autoComplete="off" 
          value={deckTitle} onChange={(e) => setDeckTitle(e.target.value)}/>
        </div>
...
<>
    ...
    className = {styles.listDeckMobile}> {deckData.title.slice(0,25) +" ⚙️"}
    </div>
    ...
    <Modal
      ...
      >
        <Fade in={openModal}>
          ...
          {searchResults.slice(0,5).map((questionsMapped: { question: string, id:string;},idx) => (
      <div onClick={() => changeCard(questions.findIndex((q: { id: string; }) => q.id === questionsMapped.id),false)} 
